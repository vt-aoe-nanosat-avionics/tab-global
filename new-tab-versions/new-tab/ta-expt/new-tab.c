...
int bootloader_write_page(rx_cmd_buff_t* rx_cmd_buff) {
    if (
  ...
  if(
   ...
   tx_cmd_buff_o->empty
    ...
    switch(rx_cmd_buff_o->data[OPCODE_INDEX]) {
        ...
        for (i = PLD_START_INDEX; i < ((size_t)0x4e); i++) {
            tx_cmd_buff_o->data[i] = ((uint8_t)0x00);
        }
        break;
    ...
    case APP_SET_TIME_OPCODE:
        ...
        // collect bytes
        uint8_t sec_0 = rx_cmd_buff_o->data[PLD_START_INDEX + 0]; // LSB
        uint8_t sec_1 = rx_cmd_buff_o->data[PLD_START_INDEX + 1];
        uint8_t sec_2 = rx_cmd_buff_o->data[PLD_START_INDEX + 2];
        uint8_t sec_3 = rx_cmd_buff_o->data[PLD_START_INDEX + 3]; // MSB
        uint8_t ns_0 = rx_cmd_buff_o->data[PLD_START_INDEX + 4]; // LSB
        uint8_t ns_1 = rx_cmd_buff_o->data[PLD_START_INDEX + 5];
        uint8_t ns_2 = rx_cmd_buff_o->data[PLD_START_INDEX + 6];
        uint8_t ns_3 = rx_cmd_buff_o->data[PLD_START_INDEX + 7]; // MSB
        // initialize common variables to known values
        sec = 0;
        ns = 0;
        success = 0;
    ...
    case COMMON_ACK_OPCODE:
      ...
      case BOOTLOADER_WRITE_PAGE_OPCODE:
        if(bootloader_running()) {
          success = bootloader_write_page(rx_cmd_buff_o);
          if(success) {
            tx_cmd_buff_o->data[MSG_LEN_INDEX] = ((uint8_t)0x07);
            tx_cmd_buff_o->data[OPCODE_INDEX] = BOOTLOADER_ACK_OPCODE;
