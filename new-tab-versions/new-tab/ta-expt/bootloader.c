...
#include <new-tab.h>        // TAOLST protocol macros, typedefs, fnctns
...
int handle_bootloader_write_page_addr32(rx_cmd_buff_t* rx_cmd_buff) {
    if (
        ...
        // write data
        for (size_t i = 0; i < BYTES_PER_WORD; i += 8) {
            ...
            if ((i + start_addr) % BYTES_PER_PAGE == 0) {
                flash_erase_page((i + start_addr) / BYTES_PER_PAGE);
